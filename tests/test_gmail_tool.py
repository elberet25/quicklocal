...
Run from the project root:
    pytest tests/test_gmail_tool.py -v
"""
...
class TestFetchMessages:
    def test_returns_parsed_list(self):
        ...
        results = GmailBaseTool._fetch_messages(service, query="in:inbox", max_results=5)
    ...
    def test_empty_result(self):
        service = MagicMock()
        service.users().messages().list().execute.return_value = {}
        results = GmailBaseTool._fetch_messages(service, query="nothing", max_results=5)
        assert results == []
    ...
    def test_respects_max_results(self):
        ...
        GmailBaseTool._fetch_messages(service, query="in:inbox", max_results=3)
...
class TestPreviewDraftReplyTool:
    def setup_method(self):
        self.tool = PreviewDraftReplyTool()

    def test_returns_json_string(self):
        result = self.tool.execute(to="to@example.com", subject="Re: Hello", body="Thanks!")
    ...
    def test_fields_are_present(self):
        result = self.tool.execute(to="to@example.com", subject="Re: Hello", body="Thanks!", reply_to_id="msg123")
        data = json.loads(result["result"])
    ...
    def test_no_reply_to_id_shows_new_thread(self):
        result = self.tool.execute(to="to@example.com", subject="Hi", body="Body")
        data = json.loads(result["result"])
        assert "new thread" in data["reply_to_id"]

    def test_does_not_call_gmail_api(self):
        with patch.object(GmailBaseTool, "_get_gmail_service") as mock_service:
            self.tool.execute(to="to@example.com", subject="Subject", body="Body")
...
class TestCreateDraftReplyTool:
    def setup_method(self):
        self.tool = CreateDraftReplyTool()
    ...
    def test_returns_json_with_draft_id(self):
        ...
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(to="to@example.com", subject="Subject", body="Body")
        data = json.loads(result["result"])
    ...
    def test_result_includes_to_and_subject(self):
        ...
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(to="recv@example.com", subject="My Subject", body="Body")
        data = json.loads(result["result"])
    ...
    def test_reply_with_threading(self):
        ...
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(to="to@example.com", subject="Re: Hi", body="Body", reply_to_id="msg001")
        data = json.loads(result["result"])
        assert data["status"] == "Draft saved"
    ...
    def test_reply_without_thread_id_still_works(self):
        service = self._make_service()
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(to="to@example.com", subject="Subject", body="Body", reply_to_id="")
        data = json.loads(result["result"])
        assert data["status"] == "Draft saved"
...
class TestReadLatestEmailsTool:
    def setup_method(self):
        self.tool = ReadLatestEmailsTool()
    ...
    def test_returns_json_list(self):
        ...
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(max_results=1)
        data = json.loads(result["result"])
...
class TestSearchEmailsTool:
    def setup_method(self):
        self.tool = SearchEmailsTool()

    def test_passes_query_to_api(self):
        service = MagicMock()
        service.users().messages().list().execute.return_value = {}
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            self.tool.execute(query="from:boss@example.com")
        call_kwargs = service.users().messages().list.call_args.kwargs
    ...
    def test_returns_json_list(self):
        ...
        with patch.object(GmailBaseTool, "_get_gmail_service", return_value=service):
            result = self.tool.execute(query="subject:invoice")
        data = json.loads(result["result"])
        assert isinstance(data, list)
